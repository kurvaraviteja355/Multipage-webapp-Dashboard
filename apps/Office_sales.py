...
predicted_data = pd.read_csv(DATA_PATH.joinpath("weekly_office_predictions.csv"))

#predicted_data = pd.read_csv('Predicted_data/weekly_office_predictions.csv')
...
fig = go.Figure(
        ...
        layout=go.Layout(
            title = {'text' : 'Office Products prediction for next two weeks', 
...
layout =  dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.H1('Microsoft Office products prediction Dashborad', className = 'text-center text-primary'),
    ...
    dbc.Row([
        ...
        dbc.Col([
            dcc.Graph(id = 'office_graph_country', figure = fig, style={'float': 'right','margin': 'auto'})
            ], width = {'size' : 5}),


        dbc.Col([
            dcc.Dropdown(id = 'office_City-dropdown', multi=False, value ='Berlin',
            options = [{'label': i, 'value': i} for i in data['Reseller_City'].unique()]),
            dcc.Graph(id = 'office_bar-garph-city', figure={ }, style={'float': 'right','margin': 'auto'})
     ...
     dbc.Row([
        ...
        dbc.Col([
            dcc.Dropdown(id='office_City-selector', multi=False, options= onLoad_cities_options()),
            dcc.Dropdown(id='office_product-selector',  multi=False),
            dcc.Graph(id = 'office_Results-table', figure={}),
            dcc.Graph(id = 'office_Graph-plot of sales', figure = {})
            ], width = {'size' : 11}),
        ], className="g-1", justify='around')
...
@app.callback(
    [Output(component_id='office_bar-garph-city', component_property='figure'),
    Output(component_id = 'office_product-selector', component_property='options'),
    Output(component_id = 'office_Results-table', component_property='figure'),
    Output(component_id = 'office_Graph-plot of sales', component_property= 'figure'),],
    [Input(component_id='office_City-dropdown', component_property='value'),
    Input(component_id = 'office_City-selector', component_property='value'),
    Input(component_id = 'office_City-selector', component_property= 'value'),
    Input(component_id = 'office_product-selector', component_property= 'value')]
