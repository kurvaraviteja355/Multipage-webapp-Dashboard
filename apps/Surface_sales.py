...
predicted_data = pd.read_csv(DATA_PATH.joinpath("weekly_surface2.csv"))


#predicted_data = pd.read_csv('Predicted_data/weekly_surface2.csv')
...
fig = go.Figure(
        ...
        layout=go.Layout(
            title = {'text' : 'Surface devices predictions for next two weeks', 
...
def onLoad_cities_options():
    ...
    city_options = (
        [{'label': city, 'value': city}
         for city in predicted_data['Reseller_City'].unique()]
    )
    return city_options
...
layout =  html.Div(dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.H1('Microsoft Surface devices predictions Dashborad', className = 'text-center text-primary'),
    ...
    dbc.Row([
        ...
        dbc.Col([
            dcc.Graph(id = 'surface_graph_country', figure = fig, style={'float': 'right','margin': 'auto'})
            ], width = {'size' : 5}),


        dbc.Col([
            dcc.Dropdown(id = 'surface_City-dropdown', multi=False, value ='Berlin',
            options = [{'label': i, 'value': i} for i in predicted_data['Reseller_City'].unique()]),
            dcc.Graph(id = 'surface_bar-garph-city', figure={ }, style={'float': 'right','margin': 'auto'})
    ...
    dbc.Row([
        ...
        dbc.Col([
            dcc.Dropdown(id='surface_City-selector', multi=False, options= onLoad_cities_options()),
            dcc.Dropdown(id='surface_product-selector',  multi=False),
            dcc.Graph(id = 'surface_Results-table', figure={}),
            dcc.Graph(id = 'surface_Graph-plot of sales', figure = {})
            ], width = {'size' : 11}),
        ], className="g-1", justify='around')
...
@app.callback(
    [Output(component_id='surface_bar-garph-city', component_property='figure'),
    Output(component_id = 'surface_product-selector', component_property='options'),
    Output(component_id = 'surface_Results-table', component_property='figure'),
    Output(component_id = 'surface_Graph-plot of sales', component_property= 'figure')],
    [Input(component_id='surface_City-dropdown', component_property='value'),
    Input(component_id = 'surface_City-selector', component_property='value'),
    Input(component_id = 'surface_City-selector', component_property= 'value'),
    Input(component_id = 'surface_product-selector', component_property= 'value')]

)

def update_graph(selected_city, city, Area, item):

    city_data = predicted_data[predicted_data["ds"] >= '2021-10-01']
    city_data = predicted_data[predicted_data["Reseller_City"] == selected_city]
    ...
    #### load the products fopr the selected city 

    products_df = predicted_data.loc[predicted_data['Reseller_City'] == Area]

    product = [{'label' : product, 'value': product} for product in products_df['Business_Unit'].unique()]


    #### Load the table in the web app
